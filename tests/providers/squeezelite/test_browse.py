...
class TestHandlePlaylistControl:
    ...
    @pytest.mark.asyncio
    async def test_delete_removes_matching_items(self):
        """Handler removes items from queue by URI when cmd is 'delete'."""
        mass = _make_mass_mock()
        queue = MagicMock()
        queue.queue_id = "queue_1"
        mass.player_queues.get_active_queue = MagicMock(return_value=queue)

        # Simulate queue items with URIs
        item1 = MagicMock()
        item1.uri = "test://track/1"
        item1.queue_item_id = "qi_1"
        item2 = MagicMock()
    ...
    async def test_delete_no_queue_items_returns_zero(self):
        """Handler returns count 0 when queue is empty."""
        mass = _make_mass_mock()
        queue = MagicMock()
        queue.queue_id = "queue_1"
        mass.player_queues.get_active_queue = MagicMock(return_value=queue)
